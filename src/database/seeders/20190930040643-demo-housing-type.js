...
module.exports = {
  up: queryInterface => {
    const names = ['Host Family', 'Residência Estudantil', 'Apartamento', 'Hotel', 'Hostel']
    const descriptions = [
      ...
      'Apartamento\',\'Essa opção é um meio termo entre dormitórios universitários e Host Family.  O estudante escolhe por alugar um apartamento sozinho ou dividir o aluguel e todas as outras contas com outros estudantes. Isso dependerá do que o intercambista deseja e o que ele pode pagar. Em caso de compartilhar o apartamento, normalmente é pago o valor por um quarto, que pode ser individual ou compartilhado, e geralmente áreas como banheiro, cozinha e sala são compartilhadas. Para encontrar esses apartamentos, o intercambista pode pesquisar em imobiliárias da região ou em grupos nas redes sociais, os grupos geralmente oferecem muitas opções para compartilhar.',
      'Geralmente é fácil encontrar hotéis próximos as escolas, porém  esse tipo de acomodação são considerados mais caras e o intercambista vai viver uma vida de turista, totalmente fora do ambiente estudantil. É pago por diária e tem acesso as facilidades que o hotel oferece.',
      'O termo Hostel é muito famoso principalmente entre os mochileiros, que viajam de país á país.  Essa é uma opção parecida com hotel, mas aqui tudo é compartilhado, banheiro, cozinha e quartos (salvo exceções, com opção de quarto individual). Se hospedar em hostel pode ser uma boa escolha para quem faz curso de curta duração, ou está a procurar de outra acomodação definitiva.'
    ]
    const data = []

    for (let i = 0; i < names.length; i++) {
      data.push({ id: i + 1, name: names[i], description: descriptions[i] })
    }
  ...
  down: queryInterface => {
    return queryInterface.bulkDelete('HousingTypes', null, {})
  }
}
